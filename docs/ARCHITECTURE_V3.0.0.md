# TrendForge Architecture v3.0.0 (MVP 3.0 - Prophet)

**版本**: 3.0.0  
**日期**: 2026-01-14

---

## 🏗️ 系统架构（MVP 3.0）

### 整体架构图

```
┌────────────────────────────────────────────────────────────────────────┐
│                        Frontend (Next.js 14)                           │
│  ┌───────────────────┐  ┌──────────────────────────┐                  │
│  │ Home Page         │  │ Analysis Page             │                  │
│  │ /                 │  │ /analysis/[channelId]     │                  │
│  └─────────┬─────────┘  └─────────────┬────────────┘                  │
│            │                           │                               │
│            │  POST /api/analyze        │ GET /api/analysis/[channelId] │
│            └───────────────┬───────────┴───────────────┬──────────────┘
│                            │                           │
│                     Prisma DB (PostgreSQL)             │
│                   Channel.fingerprint.v2_analysis       │
│                 ChannelTrend.recommendationData         │
└────────────────────────────┼───────────────────────────┘
                             │ HTTP/REST
┌────────────────────────────┼───────────────────────────┐
│                   Backend (FastAPI)                     │
│  ┌───────────────────────────────────────────────────┐  │
│  │ /api/v2/full-analysis                              │  │
│  │  - Step 1 Deep channel analysis                    │  │
│  │  - Step 2 Social trends (Twitter/Reddit/Google/Serp)|  │
│  │  - Step 3 Recommendations + title generation        │  │
│  │  - Step 4 Backtest (>=50 videos if available)       │  │
│  │  - Step 5 Prophet predictions (7-day forecast)      │  │
│  └───────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────┐  │
│  │ /api/v3/predict-trends                             │  │
│  │  - TrendPredictionEngine (Prophet)                 │  │
│  │  - peak_day / peak_score / confidence / intervals  │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```

---

## 🔄 数据流（重点：预测数据如何落库并展示）

### A. 新分析（点击“分析”）

1. 前端 `POST /api/analyze`
2. Next API 路由请求后端 `POST /api/v2/full-analysis`
3. 后端返回：
   - `trend_predictions`（数组）
   - `emerging_trends`（数组）
   - `recommendations[].prediction`（每条推荐的预测摘要）
4. 前端将预测写入 DB：
   - `Channel.fingerprint.v2_analysis.trend_predictions`
   - `Channel.fingerprint.v2_analysis.emerging_trends`
   - `ChannelTrend.recommendationData.prediction`
5. 页面 `GET /analysis/[channelId]` 展示：
   - **趋势预测图表**：`TrendPredictionChart`（黄色“预计峰值”卡片）
   - **推荐卡片**：紫色预测卡片（含“峰值: 第X天”）
   - **新兴趋势**：黄色提示（“预计第X天达到峰值”）

### B. 旧数据/缓存（你看到 55% 的根因）

- 分析结果页面通常从 DB 的 `fingerprint.v2_analysis` 读取。
- 如果 DB 中存的是旧算法生成的低置信度预测（如 55%），页面会继续显示旧值。

**MVP 3.0.0 的修复策略：**
- 在 `GET /api/analysis/[channelId]` 中，当检测到：
  - 已存预测最低置信度 `< 75`，或
  - 算法版本不匹配（`predictions_algo_version`）
  会自动调用后端 `/api/v3/predict-trends` 重新生成，并写回 DB。

---

## 🔮 Prophet 预测（MVP 3.0.0 关键约定）

- **输出字段**：`confidence`（0-100）、`peak_day`（1-7）、`peak_score`、`predictions[]`（含上下置信区间）
- **最低置信度策略**：服务端置信度算法已调整为 **≥75%**（用于产品展示与行动建议）
- **前端展示**：
  - 图表卡片右上角“预测置信度”
  - 黄色高亮“预计峰值”
  - 推荐卡片“峰值: 第X天”

---

## ♻️ 预测刷新与前端加载控制（2026-01-14 更新）

- **预测自动刷新策略**：
  - `frontend/app/api/analysis/[channelId]/route.ts` 中根据 `predictions_algo_version` 与最小置信度自动决定是否调用 `/api/v3/predict-trends` 重新生成预测并落库
  - 使用进程内 `predictionRefreshPromise` 作为 in-flight 锁，避免同一频道并发刷新导致的重复计算与长时间阻塞
- **结果页加载控制**：
  - 结果页 `analysis/[channelId]/page.tsx` 使用 `useRef` guard 防止 React StrictMode 下的双重 `fetchResults` 调用
  - 对预测刷新请求增加 30s 超时，保证即使后端预测偏慢也不会让 UI 无限 loading

---

## 🎨 UI/UX 设计（MVP 3.0.0 更新）

### 推荐详情弹窗 Tab 设计

- **Tab 1: 详细信息**：显示 AI 生成标题、表现预测、相关信息等原有内容
- **Tab 2: 7天趋势预测**：显示完整的 `TrendPredictionChart` 组件
  - 7 天趋势图表（含置信区间）
  - 趋势方向、强度、置信度
  - 峰值时机预测
  - 模型准确度指标（MAE, RMSE, MAPE）
  - 每日详细预测表格

### 去重机制

- **后端去重**：在 `predictive_recommender.py` 和 `intelligent_recommender.py` 中，对同一关键词（不区分大小写）只保留匹配度最高的推荐
- **前端去重**：在显示推荐列表时进行二次去重，确保每个关键词只显示一次

---

## ⚠️ 注意事项

- `GET /api/v3/debug-runtime` 为本地调试用端点，用于验证运行时加载的预测代码版本与置信度函数；如要生产部署建议关闭或加鉴权。
- 旧数据（在去重逻辑添加前生成的分析结果）可能仍包含重复推荐，需要重新分析频道才能看到去重效果。

